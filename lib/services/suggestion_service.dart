...
class SuggestionService {
  ...
  /// Automatically generate daily suggestion if it doesn't exist
  /// This is called when the app opens to ensure a suggestion exists
  static Future<Suggestion?> autoGenerateDailySuggestion(
    String userId,
  ) async {
    try {
      // Check if today's suggestion already exists
      final existingSuggestion = await getTodaySuggestion(userId);
      if (existingSuggestion != null) {
        if (kDebugMode) {
          debugPrint('Today\'s suggestion already exists');
        }
        return existingSuggestion;
      }

      // Get user's wardrobes
      final wardrobes = await _firestore
          .collection('users/$userId/wardrobes')
          .get();

      if (wardrobes.docs.isEmpty) {
        if (kDebugMode) {
          debugPrint('No wardrobes found for user');
        }
        return null;
      }

      // Use the first wardrobe (or you can implement logic to pick the best one)
      final wardrobeDoc = wardrobes.docs.first;
      final wardrobeData = wardrobeDoc.data();
      final now = DateTime.now();
      final wardrobe = Wardrobe(
        id: wardrobeDoc.id,
        title: wardrobeData['title'] ?? 'Wardrobe',
        location: wardrobeData['location'] ?? '',
        season: wardrobeData['season'] ?? 'All-season',
        createdAt: (wardrobeData['createdAt'] as Timestamp?)?.toDate() ?? now,
        updatedAt: (wardrobeData['updatedAt'] as Timestamp?)?.toDate() ?? now,
      );

      // Generate suggestion
      if (kDebugMode) {
        debugPrint('Auto-generating daily suggestion for wardrobe: ${wardrobe.id}');
      }
      ...
      if (kDebugMode) {
        debugPrint('Failed to auto-generate daily suggestion: $e');
      }
      return null;
    }
  }
